...
import type { AnimeFlowNode, AnimeNodeData } from '../../types'
...
/**
 * Position every node by running dagre per franchise (so each one gets its
 * own left-to-right chronological row), then stacking the franchises
 * vertically with a fixed gap so independent stories never touch.
 *
 * Returns xyflow-shaped nodes ready to feed into <ReactFlow nodes={...} />.
 */
